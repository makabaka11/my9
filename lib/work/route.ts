...
const SEARCH_RATE_LIMIT_MAX_REQUESTS = 6;
...
type SearchItems = Awaited<ReturnType<typeof searchWorkSubjects>>;
...
function getSearchMemoryStore(): SearchMemoryStore {
  const g = globalThis as typeof globalThis & {
    __MY9_WORK_SEARCH_MEMORY__?: SearchMemoryStore;
  };

  if (!g.__MY9_WORK_SEARCH_MEMORY__) {
    g.__MY9_WORK_SEARCH_MEMORY__ = {
  ...
  return g.__MY9_WORK_SEARCH_MEMORY__;
}
...
function toSearchCacheKey(query: string) {
  return `work:${normalizeSearchQuery(query)}`;
}
...
function checkSearchRateLimit(request: Request): {
  limited: boolean;
  ...
  if (existing.count >= SEARCH_RATE_LIMIT_MAX_REQUESTS) {
    ...
    if (memory.rateLimitBlockedCount <= 5 || memory.rateLimitBlockedCount % 50 === 0) {
      console.warn(
        `[work-search-rate-limit] blocked=${memory.rateLimitBlockedCount} retry=${retryAfterSeconds}s`
      );
    }
    return {
      limited: true,
...
async function getCachedSearchItems(query: string): Promise<SearchItems> {
  const memory = getSearchMemoryStore();
  const key = toSearchCacheKey(query);
  const now = Date.now();
  ...
  const requestPromise = searchWorkSubjects({ query });
...
export async function handleWorkSearchRequest(request: Request) {
